...
def lineintegral(a, b, Nq, g):
    ...
    # The norm of b- a to scale with
    norm = np.linalg.norm(b - a, ord=2)
    # f times local basis function, f parameterized
    f1 = lambda t: g(0.5*((b[0]-a[0])*t + (a[0]+b[0])),0.5*((b[1]-a[1])*t + (a[1]+b[1])))*(1+t)/2
    f2 = lambda t: g(0.5*((b[0]-a[0])*t + (a[0]+b[0])),0.5*((b[1]-a[1])*t + (a[1]+b[1])))*(1+t)/2
    # The Integral as the sum of two integrals times norm over 2
    I = (np.sum(rho_q * f1(z_q)) + np.sum(rho_q * f2(z_q))) * norm / 2
    return I
